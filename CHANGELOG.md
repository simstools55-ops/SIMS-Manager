## v6.7.13
- 「未完了の作業を再開」で、同一記事に現行Doctor/利用者確認Workflowがある場合、古い「aDoctor診断後のモニター終了・完了登録」を重複表示しないよう整理。履歴・Case自体は削除しない。
- 「未完了の作業を再開」を「設定・メンテナンス」から「SIMS今日の作業」へ移動し、「記事情報を更新」の直前に配置。
...
- v6.7.9の高速化内容と登録仕様は維持。

## v6.7.9
- aWriter登録の03dでシートreadを5回から1回へ集約。
- aWriter登録の03g新規履歴行装飾を同期処理から除外。
- v6.7.8の03x追加計測を撤去。

## v6.7.8
- aWriter改善結果登録03全体の外側8区間プロファイルを追加（計測のみ）。

## v6.7.7
- aWriter改善履歴登録の内部区間計測 03a〜03h が結果画面へ伝播しない不具合を修正。登録ロジック変更なし。

## v6.7.1

## v6.7.2
- 通常改善完了時のWorkflow削除を阻害していた誤混入profiler参照を除去。
- 改善履歴登録済み案件を未完了一覧から除外する完了境界を追加。

- 改善ナビの閉じる操作を即時化。表示前に保存済みのWorkflowを閉じる時に同期再保存していた重複処理を除去。
- 登録完了後のclose checkpointによる未完了Workflow再生成リスクを解消。

## v6.6.92
- STEP2の日次MERGE内Settings書込17回を2回のバッチ書込へ集約し、仕様を変えずI/Oを削減。

## v6.6.85
- STEP3の改善効果更新後に「記事管理」と「改善履歴」を1回だけ読み込む共通スナップショットを追加。

## v6.6.92
- STEP3完全区間計測を追加。処理プロファイル記録と処理ログ記録自体の累計時間を可視化し、未計測時間の正体を特定できるようにした。業務ロジック・判定仕様は変更なし。

## v6.6.87
- STEP3高速化。改善の推移で完了済み状態の再書込みを抑止。
- 今日の改善シート再描画で全書式クリアを廃止し、既存書式を再利用。
- DisplayedImprovementCount の重複保存を削減。

## v6.6.86
- STEP3深掘り計測を追加。改善の推移の判定計算を4区間、今日の改善整合を6区間に分解。
- 診断のみで業務ロジック・状態管理仕様は変更なし。

- 「今日の改善整合」と「未完了整理」で同じ観察終了候補を共有し、記事管理・改善履歴の重複読込と重複候補抽出を削減。
- 改善効果更新中の状態遷移を取り込むため、スナップショット生成は改善効果更新後に限定。記事管理を状態管理の正本とする設計は維持。

## v6.6.84
- 「今日の改善」再描画の重複I/Oを削減。既存レイアウトを再利用し、使用範囲のみクリアする高速経路を追加。
- 再描画直後の `sbmApplySelectionUi_()` 再実行と強制 `SpreadsheetApp.flush()` を削除。
- STEP2_TODAY_WRITE と STEP3_今日の改善整合の双方で同じ高速描画経路を使用。
- 記事管理を状態管理の正本とする設計・判定条件は維持。

## v6.6.83
- STEP2の日付3列の差分比較を型に依存しない比較へ変更。
- SheetsのDate型と文字列の日付が同一内容でも差分扱いになる問題を解消。
- 記事管理正本・更新値の意味・日次処理の判定仕様は維持。

## v6.6.81
- STEP2の記事DB差分反映を列単位の差分書込へ変更。
- 日次更新値の意味・記事管理を状態管理の正本とする設計は維持。
- 変更のない列を同一ブロックだからという理由だけで再書込する処理を廃止。

## v6.6.79
- STEP2_DB_MERGE の changedCells を列名別に計測。
- 記事管理の書き戻しロジックは変更せず、1305セル変更の内訳特定を優先。

## v6.6.78
- STEP2/記事DB MERGEのSettingsシート反復I/Oを一括化。

## v6.6.77
- 日次処理STEP2の詳細プロファイルを追加。
- 記事DB差分反映、完了記事再評価、候補選定、今日の改善シート反映、記事管理作業状態反映、設定保存の各区間を「処理プロファイル」に記録。
- 業務ロジック、候補判定、記事管理を状態管理の正本とする設計は変更なし。

# Changelog

## v6.6.76
- 日次STEP3の「改善の推移更新」内部で重複していた経過観察終了→今日の改善同期を省略。
- 同同期はSTEP3直後の「今日の改善整合」で1回だけ実行し、記事管理を状態管理の正本とする設計・判定条件は維持。
- 手動の「改善の推移更新」では従来どおり即時同期する。


## v6.6.75
- STEP3未完了整理を一括読込・索引照合・一括書戻しへ変更。
- 記事単位のDoctor_Cases / Doctor_Workflow_State再読込とdeleteRow反復を廃止。
- 既存の完了境界・経過観察終了除外・明示的新サイクル保持条件は維持。

v6.6.73
- 日次処理STEP3の区間計測を強化。改善の推移更新・今日の改善整合・未完了整理を個別記録。
- 未完了整理を25記事単位で進捗記録し、実行時間上限到達時のボトルネック特定を可能にした。
- 処理仕様・状態判定は変更なし。
## v6.6.73
- 日次STEP1の設定保存15回を一括保存へ統合し、実測11秒のI/Oボトルネックを削減。
- v6.6.68の表示高速化・記事管理正本設計は維持。

## v6.6.68
- 「今日の改善」と「未完了の作業を再開」に区間計測を追加。
- 記事管理正本の状態管理は変更せず、表示時整合・シート読込・候補抽出・UI表示のボトルネックを実測可能化。
- 高速化対象の移管判断は実測後に行うため、本版では処理ロジックの削除・日次移管は未実施。

## v6.6.66
- 今日の改善の通常候補へArticleIDを必須継承。旧保存候補も記事URLから記事管理を照合してArticleIDを補完。

## v6.6.65
- Creator Direct新規記事の4週観察終了を既存記事の再診フローから分離。
- 記事管理の内部状態列を毎回非表示化。
- 改善の推移の選択列・チェックボックスを毎回非表示/除去。

## v6.6.64
- 利用者向けシートの内部列・不要選択UIを整理。改善の推移を閲覧専用化し、今日の改善でArticleIDを可視化。

# v6.6.63

- 日次処理の週次測定を日付単位で判定し、測定予定日の何時に実行しても当日分を記録できるよう修正。
- 測定予定日に日次処理できなかった場合は、次回の日次処理で未実施の1回分を現在のGSCデータで自動補完。予定日を過去日に偽装しない。
- 「測定期限超過」を利用者向けに「測定待ち（予定日超過）」へ変更。
- 1回の日次処理で同じ実測値を複数週へ複製せず、1回だけ記録する。

## v6.6.63
- 記事管理を記事状態の唯一の正本へ統一。
- 作業理由・状態更新日・最終改善完了日・再評価予定日を追加。
- 完了後90日で未着手へ戻す循環型再評価を追加。
- 今日の改善・改善の推移の表示操作から状態変更を除去。
- 経過観察終了は記事管理の現在状態＋作業理由で管理。

## v6.6.61
- 経過観察終了候補の抽出をモニター状態文字列依存から、4回測定完了＋最終結果判定へ変更。
- 経過観察終了候補はメインクエリ未取得でも「今日の改善」から除外しない。
- v6.6.59で「改善の推移」から消えた既存案件も履歴から「今日の改善」へ復元可能に修正。

## v6.6.59
- 経過観察終了案件が「改善の推移」に残る問題を修正。表示時にも今日の改善へ移管して現役一覧から除外する。
- 経過観察終了案件を未完了データ自動削除の対象外に変更。

# v6.6.58
- 経過観察終了・要再診案件を「改善の推移」から「今日の改善」へ移管。
- 「選択記事の改善内容を見る」から従来の観察終了後処置を起動。
- 改善の推移メニューの観察終了後処置入口を削除。
- aDoctor再診のURL解決をArticleID→記事管理の正本照合へ変更。
- 観察終了後は新規Workflowとして開始し、中断時だけ未完了に残す。
...
- Case作成日時ではなく、改善の推移への移管を処置完了境界として判定。
- 経過観察終了後に「改善の推移」から正式開始した EFFECT_AFTER_OBSERVATION 再診Workflowは除外せず再開可能。
- Merge / Creator系Workflowは従来どおり別処置として保持。
...
- StarterのaDoctor利用不可、診断ロジック、License Center認証は変更なし。

# v6.6.46

- 日次処理ダイアログを事前I/Oより先に表示する方式へ変更。
- Settings反復読込を事前確認1回のMap読込へ統合。
- 事前確認の区間時間ログを追加。
- 日次処理STEP1〜3本体は変更なし。

# v6.6.45

- 「未完了の作業を再開」で、改善の推移へ正式登録済みの通常改善Workflowを除外する照合を追加。
- ArticleIDを第一キー、記事URLを補助キーとして改善の推移と照合し、登録完了済み案件が未完了一覧へ残る問題を修正。
- v6.6.44の作成時重複防止・一覧重複排除・完了時一括終了は維持。

## v6.6.44
- 通常改善Workflowの同一記事重複をArticleID/URLで統合。
- 未完了一覧を同一記事1件へ重複排除。
- 改善登録完了時に同一記事の残存通常改善Workflowを一括完了。

## v6.6.43
- 改善ナビ表示前に通常改善Workflowを同期保存し、表示成立時点で未完了案件として確定。
- 「未完了の作業を再開」で通常改善Workflowを最新1件だけでなく全件列挙。
- 通常改善とDoctor系Workflowを同一選択リストへ統合し、選択したWorkflow Identityをそのまま再開。
...
- Homeが未作成またはレイアウト更新必要時のみ再構築し、現在テーマを適用してから表示。
- v6.6.39の今日の作業メニュー 0〜3 整理を維持。
...
- Fullの改善ガイドとaWriter依頼文へ同じ指示をそのまま使用。
- 共通方針だけの抽象的なFull改善ガイドを廃止。
- Starter Editionの自己修正向け改善ガイドは変更なし。
...
- サイト健康診断はManager内蔵機能として維持。
- 未発芽・発芽・要改善および28日観察後の要見直しはStarter改善ナビへ接続。
- Full EditionのaDoctor連携動作は維持。

# v6.5.18

- 保存済みタイトルのローカル正規化を共通化。正常なタイトルは再取得せず、設定済みBlogNameの末尾混入だけをネットワークアクセスなしで補正。
- 記事情報更新で混入タイトルをローカル修正候補として検出。Web取得はタイトル欠落/プレースホルダー時のみ。
- 記事管理の軽量整形と改善ナビ表示にも同じタイトル正規化を適用。
- 「今日の改善」の詳細チェックから改善ナビが開かない回帰を修正。
- GSC、改善ガイド判定、aWriter依頼文生成・Contractは変更なし。

# v6.5.16

- GA4プロトタイプの正規化方針をSBMへ移植し、はてなブログのtitleタグ末尾ブログ名を除去。
- 記事本文取得を本文コンテナ優先へ変更し、サイト共通H1・共通見出し・関連記事等の混入を抑止。
- 旧キャッシュを分離するためメタ/本文/タイトル情報キャッシュキーを更新。
- GSC、改善判定、aWriter依頼文生成は変更なし。

## v6.5.15
- 改善ナビの成長・育成ランクで診断深度を拡張。回答位置とFAQ発見性を追加診断し、件数水増しなしで改善テーマの取りこぼしを削減。
- aWriter依頼文生成は変更なし。

# v6.5.11
- 改善候補なのに改善ガイド0件となるデッドエンドを修正。
- CTR機会が明確な場合、aWriter依頼文の優先順位に沿ってSEOタイトル/メタ（エース保護時はメタのみ）の改善ガイドを派生表示。
...
- Homeを開くたびの記事DB全読込を停止し、保存Snapshotを優先。必要時のみliveArticle再読込。
- Home Snapshotを製品バージョン更新だけで無効化しないよう変更。
- 今日の改善の保存済み候補でも、区分・改善理由・期待効果・予想時間を現行の収益優先ルールへ軽量同期。
- 区分を「エース化 / 収益改善 / 育成改善 / 安全改善」へ明確化。

## v6.4.8
- 収益最大化を最終目的とする「収益優先・安全改善ポリシー」を正本化。
- 記事ランクと「今日作業すべき優先度」を分離。
- 成長記事はエース化、エース記事はSEO骨格を保護した収益改善を基本方針とする。
- 高評価記事の変更前スナップショット、28日後のaDoctor再診、KEEP / IMPROVE / RESTORE、原状復帰後の再測定を段階実装する仕様を確定。
- 本版は設計基準の固定版であり、候補抽出・復元ランタイムの挙動変更は次版以降で段階導入する。

## v6.4.7 — 6ランク共通の記事詳細トリアージ
- 未発芽・発芽：aDoctorを優先。
- 育成：改善ナビで必要最小限の改善。
- 安定・成長・エース：保護観察を基本とし、要改善等の異常時だけaDoctorを優先。
- 詳細画面に各ランクの判断理由を表示。

## v6.4.6 — 記事詳細のSIMSトリアージ

- 記事管理の詳細で、未発芽・発芽の記事は改善ナビへ直接送らずaDoctor精密診断を優先。
- 「今日の改善」に入っている未発芽・発芽も、改善開始前なら診断を先行。
- すでに改善中・処置中の記事は既存Workflowを継続し、モニター中は効果確認を優先。
- 要確認・インデックス要確認・追加診断待ちなど既存状態の優先順位を維持。
- 記事詳細からのaDoctor起動にサーバー側トリアージ確認を追加し、任意記事の無制限診断を防止。
...
## v6.7.0
- 未完了再開で aWriter待ち案件を選択した際、選択Caseの保存済みaWriter紹介状・結果登録画面へ直接復帰するよう修正。
- 高速化・日次処理・候補抽出ロジックは変更なし。
