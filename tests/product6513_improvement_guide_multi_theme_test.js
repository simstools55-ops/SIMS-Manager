...
const meta={query:'windows11 wifi 接続できない',title:'Windows11のWi-Fiトラブル対処法',seoTitle:'Windows11のWi-Fiトラブル対処法',description:'Windows11でWi-Fiの問題を解決する方法を解説。',imps:1800,ctrText:'1.3%',posText:'7.8',topQueries:[
...
const source={introduction:'Windows11でWi-Fiに問題があるときの対処法を順番に紹介します。',sections:[
 {level:2,heading:'最初に確認すること',text:'Wi-Fiに接続できない場合は機内モードとルーターを確認します。'},
...
if(advice.length<3) throw new Error('expected multiple independent themes, got '+JSON.stringify(advice));
const joined=advice.join('\n');
if(!joined.includes('検索結果')||!joined.includes('見出し')||!joined.includes('導入文')) throw new Error('missing independent guide theme: '+joined);
if(/aWriter|Writer依頼文|利用者判断|確認してください/.test(joined)) throw new Error('internal/non-actionable wording leaked: '+joined);
console.log('PASS: independent improvement purposes are emitted as multiple user-facing guides');
