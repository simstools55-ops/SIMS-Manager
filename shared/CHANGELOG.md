## v6.7.56 - 2026-09-26
- 「aDoctor 精密診断を途中から再開」ダイアログの「対象記事」カードを、③の紹介状コピー操作の直下へ移動。処理ロジック・状態遷移は変更なし。

## v6.7.41

## v6.7.42
- 今日の改善再構築時に旧完了行の表示書式を初期化。
- 前日完了を候補プール段階で除外し、同区分の次点候補を補充できるよう修正。
- 日次STEP2のシート再描画前に「今日の改善」の完了ArticleID/URLを退避し、候補選定から除外。
- STEP3へ完了除外キーを引き継ぎ、前日完了記事の再混入を防止。空き枠は既存候補ロジックで補充。

## v6.7.26
- 「未完了の作業を再開」の操作ボタン描画を妨げていたダイアログ内JavaScript文字列不整合を修正。
- SERP参入余地チェック起点のaCreator新記事登録完了時に、起点SERP WorkflowをCOMPLETEDへ更新して未完了一覧から除外。
- 既存の再開Dispatcher、Doctor再診、通常改善、SERP判定ロジックは維持。

# v6.7.14
- 利用者確認後のaDoctor再診結果を同一の再開ダイアログで受け取り、次工程へ継続できるよう修正。
