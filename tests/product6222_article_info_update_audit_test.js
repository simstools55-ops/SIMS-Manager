...
const checks=[
  ['version 6.2.22',s.includes("const SBM_VERSION = '6.2.22';")],
  ['audit helper',s.includes('function sbmArticleInfoUpdateAudit_()')],
  ['user-facing dialog',s.includes('function sbmOpenArticleInfoUpdate()') && s.includes('記事管理全体を点検し、未取得のタイトル・メインクエリ・SEO情報を補完します。')],
  ['real query distinction',s.includes('sbmIsInferredQueryDisplay_(rawQuery)')],
  ['search exposure waiting split',s.includes('queryFetchable') && s.includes('queryWaiting')],
  ['all-db worker',s.includes('function sbmArticleInfoUpdateWorker_()')],
  ['title update',s.includes("updates['記事タイトル']=fetchedTitle")],
  ['real query update',s.includes("updates['メインクエリ']=q.replace")],
