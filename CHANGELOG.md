...
- 「記事情報を更新」の点検を不足記事のArticleID＋URL抽出方式へ単純化。
- 記事タイトル/メインクエリの空欄・未取得・推定値のみを更新対象にする。
- 点検時のWeb/GSCアクセス、H1不一致、SEOタイトル評価、表示回数分岐を除外。
- 更新前後にArticleID＋URLおよび更新前/保存後の値を照合する安全ゲートを追加。
- 日次処理・6か月GSC取得・未発芽判定は変更なし。

## v6.2.39 - 2026-09-10

- 「記事情報を更新」の点検経路を必要列だけの軽量スキャンへ変更。
- Article DB全27列の `getDataRange()` と全件汎用オブジェクト化を廃止。
- 正常タイトル行ではSEOタイトル解析を省略し、必要な行だけ評価。
- 更新対象準備も同じ軽量読込・判定を再利用。
- v6.2.38の6か月クエリ未取得記事一覧表示を維持。
- 日次処理・6か月GSC取得・1件診断・未発芽判定は変更なし。

## v6.2.38 - 2026-09-10

- 実運用で数秒点検を確認済みのv6.2.37 Code.gsを正本として再構築。
- 記事情報更新後、過去6か月でもメインクエリを取得できなかった記事をArticleID・記事タイトル・URLで一覧表示。
- 点検ロジック、日次処理、6か月取得条件、SEOタイトル・メタディスクリプション処理は変更なし。
- 未発芽判定と「1件だけ診断」削除は次工程へ分離。
