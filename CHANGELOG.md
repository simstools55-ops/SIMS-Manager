...
- 「記事情報を更新」を、記事タイトルと未取得メインクエリの補完に限定した運用確定仕様としてRepositoryへ集約。
- Article DB 200件規模の点検を一括読込＋メモリ判定へ最適化し、実測約1秒を確認。
- 未取得メインクエリは過去6か月のGSCデータから1記事ずつ取得。10件約24秒、8件更新を確認。
- ArticleID＋正規化URL＋更新前値の三重照合を維持。取得不可・安全保留・処理エラーを分離集計。
- 日次処理、SEOタイトル、メタディスクリプション等の既存処理は変更なし。
- 「未発芽」ランクとDoctor連携は次工程へ分離。
## v6.2.19 - 2026-09-10
