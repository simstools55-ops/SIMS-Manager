## v6.7.60 - 2026-09-26
- `🔄 再診処置` 候補の正本を記事管理の作業状態へ統一。
- 旧実装は同一記事の最新改善履歴だけを見ていたため、別サイクルの履歴が新しい場合に4週終了済みの再診処置を候補化できなかった。
- 改善履歴は候補生成の必須条件から外し、履歴ID・表示情報の補助参照として使用。
- 週次キャッチアップ、最終判定、Workflowは変更なし。

## v6.7.58 - 2026-09-26

## v6.7.59 - 2026-09-26
- 4週判定で新たに `🔄 再診処置` になった記事が、過去の「今日の改善」の終了/完了除外キーに一致して表示されない問題を修正。
- 終了/完了除外は通常候補の再掲防止には維持し、`EFFECT_AFTER_OBSERVATION`（再診処置）とエース急落は優先候補として除外対象外にした。
- v6.7.58の週次測定キャッチアップ、最終判定、Workflowには変更なし。
- 期限超過した週次測定を、1回の日次処理で到来済み回数まで連続キャッチアップするよう修正。
- 長期未実行案件が「1週／4週」「測定待ち（予定日超過）」に残り続ける問題を解消し、4回到達後は既存仕様どおり「完了」または「再診処置」へ遷移。
- 効果判定ロジック・Workflow・今日の改善優先順位は変更なし。

## v6.7.57 - 2026-09-26
- 「今日の改善」0件ダイアログの「健康診断へ進む」を正式な `sbmDoctorRunHealthCheck` へ接続し、開始表示のまま停止する不具合を修正。健康診断本体・候補判定は変更なし。

## v6.7.56 - 2026-09-26
- 「aDoctor 精密診断を途中から再開」ダイアログの「対象記事」カードを、③の紹介状コピー操作の直下へ移動。処理ロジック・状態遷移は変更なし。

## v6.7.55 - 2026-09-26
- 完了記事の90日管理情報が欠落している場合、最新の完了済み改善履歴の4回目測定日時から「最終改善完了日」「再評価予定日」を安全に補完。日付を確定できない記事は変更しない。

## v6.7.54 - 2026-09-26
- 日次STEP3の作業状態整合を双方向化。
- 有効な通常改善Workflowがある「未着手」記事を「改善中」へ復旧し、未完了再開と記事管理の不整合を解消。
- Doctor／Merge系や正式登録済み改善サイクルの扱いは変更なし。
- リリース関連表記をv6.7.54へ同期。

## v6.7.53 - 2026-09-26
- 「今日の改善」の表示優先順を `🚨 急落 → 🔄 再診処置 → 通常改善候補` に固定。
- 描画直前で優先案件を安定ソートし、通常カテゴリ内の既存順序は維持。
- リリース関連表記をv6.7.53へ同期。

## v6.7.52 - 2026-09-26
- 表示テーマをモノトーン固定へ統一し、テーマ選択UIを廃止。
- Home生成時にモノトーン書式を完成させ、通常表示時のテーマ再描画を停止。
- リリース関連表記をv6.7.52へ同期。

## v6.7.51 - 2026-09-26
- Homeをアクティブ化する前に選択テーマを確定し、標準配色が一瞬表示されるちらつきを抑止。
- Home以外の表示テーマ処理やデータ更新処理は変更しない。
- リリース関連表記をv6.7.51へ同期。

## v6.7.50 - 2026-09-26
- Homeレイアウト署名を現行配置へ同期。正常なHomeの不要な再構築を解消。
- Homeを開く際の一時的な標準配色への復帰と、その後のテーマ再適用を防止。
- リリース関連表記をv6.7.50へ同期。
...
- 正常な作業中案件を誤って戻さない保守的な判定とし、区間計測ログを追加。

## v6.7.47 - 2026-09-26
- 記事管理を作業状態の正本として整理。
- `🚨 急落` / `🔄 再診処置` を明示状態として追加。
- 旧 `🔥 今日の改善` を改善履歴から `🔄 再診処置` または `未着手` へ移行。
- `🛠️ 処置中` 等を `✏️ 改善中` へ統合。
- 経過観察終了後の再診は既存 `EFFECT_AFTER_OBSERVATION` を継続利用。


## v6.7.44
- 今日の改善の完了記事が日次処理で即再候補化される問題を修正。
- 改善履歴の改善日ではなく、記事管理の明示的な90日再評価解禁を再候補化条件に統一。
# Changelog

## v6.7.45
- エース記事の急落監視を追加。更新前／更新後エースを直近28日と前28日で比較する。
- 複合条件を満たす急落を `🚨 エース急落` として通常カテゴリとは別枠で「今日の改善」へ優先追加。
- エース急落からaDoctor精密診断へ直接接続し、診断前の自動記事修正は行わない。
- 完了エースは完了後14日を保護し、その後の急落時だけ `未着手 / GSC急落再評価` として90日保護を例外解除。
- 通常記事の完了除外・90日再評価・経過観察終了の既存ルートは維持。

## v6.7.43
- 改善履歴の最新サイクルを通常候補選定時に照合し、ACTIVEおよび完了後90日以内の記事の再候補化を防止。
- REVIEW_REQUIREDは経過観察終了後の再確認ルートを維持。
- 候補評価基準・4区分×最大2件・次点補充は変更なし。
...
## v6.7.40
- 日次STEP3「今日の改善」再構築の前後に診断ログを追加。候補ロジック・書込みロジックは変更なし。

## v6.7.35 - 2026-09-24

## v6.7.39
- 日次STEP3で「今日の改善」を最新の記事管理状態から再構築し、前日完了行の残存と補充漏れを修正。

## v6.7.38
- 日次処理後の「今日の改善」を最新状態へ整合。完了・進行中記事を除外し、空き枠を既存候補選定ロジックで補充。
- 候補選定ルール、高速化済み処理、未完了再開ロジックは変更なし。

## v6.7.37
- 「未完了の作業を確認しています…」中央表示に回転インジケータを追加。
- 未完了0件の結果をtoastではなく中央モーダルで明示。
- 未完了探索ロジック、候補選択画面、高速化済み処理は変更なし。

- 「未完了の作業を再開」の確認中表示を右下toastから中央modeless dialogへ変更。
- 未完了0件時に中央ダイアログで結果を明示。
- 候補探索・再開選択・高速化済み処理は変更なし。

## v6.7.34 - 2026-09-24
- aWriter結果の `COMPLETED_WITH_FOLLOW_UP` を処置完了状態として受理。
- 未知ステータス扱いで `aWriter結果再登録待ち` に戻る不具合を修正。
- 既存のMERGE follow-up判定は維持し、非MERGE follow-upの新規自動Workflowは追加しない。
- 高速化済みの登録・モニタリング経路には変更なし。

## v6.7.33 - 2026-09-24
- 「今日の改善」の完了表示を一方向に保護。既存の「完了」行を通常候補へ戻さない。
- 過去版で選択セルだけチェックボックスへ戻った場合も、タイトルの取り消し線を完了表示の既存証跡として復元する。
- A000033の完了同期を維持しつつ、A000028/A000019のような完了済み行の回帰を防止。
- 全シート再構築・GSC取得・全体書式再適用は追加しない。
- 「今日の改善」の通常表示経路では高速化のため完了表示同期が呼ばれていなかった問題を修正。
- 表示テーマ適用後に、記事管理でモニター中/完了の対象行だけをArticleID優先で軽量グレーアウト。
- 固定候補リスト、候補再選定、全シート再描画は行わず高速化を維持。

## v6.7.30
- 「今日の改善」の完了済み表示を ArticleID 優先で記事管理と同期。
- 完了済み行を選択不可・グレーアウトし、URL依存による同期漏れを防止。
- 対象候補だけの軽量照合で既存高速化を維持。

## v6.7.29
- aDoctor経過観察終了の完了登録を厳格化し、対象案件の状態整合を確認してから成功を返すよう修正。
- 完了対象を「今日の改善」へ即時同期し、描画時にも記事管理正本から完了済み候補を軽量除外。
- 全体再構築を避け、既存高速化を維持。

## v6.7.28
- 「今日の改善」のArticleID列を記事管理正本から軽量補正し、文章等の誤値を修復。
- 保存候補のArticleIDもURL一致で同期し、再描画時の再混入を防止。
- Code.gs先頭の累積改修ログを整理し、過去履歴をCHANGELOG / Release Notesへ集約。
- 全シート再構築・全書式再適用は行わず、既存高速化を維持。

## v6.7.27
- 改善の推移の登録後整形を対象1行のみに限定し、背景表示と高速化を維持。
- 操作系ダイアログの中央表示を復元。
...
## v6.7.25
- 「未完了の作業を再開」の全候補に「取りやめる」を追加。
- 既存データを削除せず、Workflow / Case を利用者取消状態として未完了一覧から除外。
- 整理候補専用の旧案件整理機能は維持。

## v6.7.24
- SERP上位10件の一部が取得不能でも、取得済み結果で参入余地判定を継続するよう変更。
- 10件未満の場合は確認件数・データ不足の注意・補正後確信度を判定画面に表示。
- 旧「上位30件」進捗表示を現行の上位10件仕様へ統一。
- SERP結果が0件の場合のみ判定不能として停止。
...
- 高速化・未完了再開・Doctor再診関連は変更なし。

## v6.7.20
- 保存済みDoctor再診依頼の旧V2契約版を、同一V2形式・正規送受信元に限って現行契約へ安全に昇格。
- FOLLOW_UP_REQUEST_READYを「aDoctor再診結果の登録」として表示し、既取得の再診結果をSIMSへ戻せるよう修正。

## v6.7.17
- 利用者確認後に生成したDoctor再診Caseを未完了Workflowの正本として扱うよう修正。
- 旧Caseに残ったFOLLOW_UP_REQUEST_READYを新Caseへ移管し、旧Caseを履歴保持のまま再開候補から除外。
- v6.7.11〜v6.7.16で作成済みの旧データも、未完了一覧表示時に局所補正。

## v6.7.16
- 「未完了の作業を再開」の二重ダイアログ方式を廃止し、Core同期実行＋結果画面表示へ単純化。
- 実測約8秒で完了する候補抽出後に待機画面だけが残るUI競合を解消。
- Workflow候補抽出・Case判定・Doctor再診ロジック自体は変更なし。
