## v5.21.45-dev
- 日次STEP2を狭い列ブロックの差分更新へ変更。
- 記事DB全件clearContent・全列再書込・全件背景色再描画を通常日次から除外。
- 記事DB初回構築を100記事単位で継続可能に変更。
- STEP2に安全な継続フェーズを追加。

# v5.21.44-dev

- 完了済みダイアログボタンを非表示化し、次工程または閉じる操作だけを残すUIへ統一。
- 日次処理ロジックはv5.21.43から変更なし。


## v5.21.43-dev
- 日次STEP2の性能回帰を修正。v5.21.41/v5.21.42の性能変更を撤回し、v5.21.40の実測安定ロジックへ復帰。
- 日次STEP2から外部記事情報補完を分離したv5.21.40の改善は維持。
# Changelog

## 5.21.42-dev - 2026-09-03
- 日次STEP2の全消去・全背景色再描画を廃止。
- 記事DBマージ結果を候補選定へメモリ渡しし、再読込を削減。
- Settingsの多数の個別I/Oを一括更新へ変更。
- バージョン表示・メタデータ一式を5.21.42へ同期。
